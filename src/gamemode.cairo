...
pub mod LogicQuestGameMode {
    ...
    use quest_contract::base::types::{
        GameMode, GameModeType, UserGameModeProgress, GameModeAchievement
    };
    ...
    struct Storage {
        ...
        // User progress
        user_progress: Map<(ContractAddress, u32), UserGameModeProgress>, // Maps (user, mode_id) to progress
        ...
        // Active sessions
        active_sessions: Map<(ContractAddress, u32, u32), u64>, // Maps (user, puzzle_id, mode_id) to start_timestamp
    }
    ...
    impl GameModeImpl of IGameMode<ContractState> {
        ...
        fn create_game_mode(
            ...
            // Emit event
            self.emit(Event::GameModeCreated(GameModeCreated { 
                mode_id, name, mode_type 
            }));
        ...
        fn update_game_mode(
            ...
            // Emit event
            self.emit(Event::GameModeUpdated(GameModeUpdated { 
        ...
        fn enable_game_mode(ref self: ContractState, mode_id: u32) {
            ...
            // Emit event
            self.emit(Event::GameModeStatusChanged(GameModeStatusChanged { 
                mode_id, enabled: true 
            }));
        }
        ...
        fn create_achievement(
            ...
            // Emit event
            self.emit(Event::AchievementCreated(AchievementCreated { 
                achievement_id, mode_id, name 
            }));
        ...
        fn start_puzzle_in_mode(
            ...
            let adjusted_time_limit = self._calculate_adjusted_time(mode_id, 0); // Time will be passed from quest contract
            
            // Emit event
            self.emit(Event::PuzzleStarted(PuzzleStarted { 
                user, 
                puzzle_id, 
                mode_id, 
                adjusted_time_limit, 
        ...
        fn complete_puzzle_in_mode(
            ...
            let adjusted_rewards = self.calculate_reward_modifier(mode_id, points_earned, time_taken);
            ...
            // Emit event
            self.emit(Event::PuzzleCompleted(PuzzleCompleted { 
                user, 
                puzzle_id, 
                mode_id, 
                base_points: base_rewards, 
                adjusted_rewards 
            }));
        ...
        fn get_user_progress(self: @ContractState, user: ContractAddress, mode_id: u32) -> UserGameModeProgress {
        ...
        fn calculate_reward_modifier(self: @ContractState, mode_id: u32, points: u32, time_taken: u32) -> u32 {
            assert(mode_id <= self.game_modes_count.read(), 'Invalid mode ID');
            let mode = self.game_modes.read(mode_id);
    ...
    impl InternalFunctions of InternalFunctionsTrait {
        ...
        fn _create_default_achievements(
            ...
            self.mode_achievements.write((challenge_id, challenge_ach_count), challenge_achievement_id);
            self.mode_achievement_count.write(challenge_id, challenge_ach_count + 1);
            self.mode_achievements.write((challenge_id, challenge_ach_count + 1), challenge_achievement_id2);
            ...
            self.mode_achievements.write((time_attack_id, time_attack_ach_count), time_attack_achievement_id);
            self.mode_achievement_count.write(time_attack_id, time_attack_ach_count + 1);
            self.mode_achievements.write((time_attack_id, time_attack_ach_count + 1), time_attack_achievement_id2);
        ...
        fn _check_achievements(
            ...
            loop {
                ...
                if !already_earned {
                    ...
                    if achievement.condition_type == 1 && progress.completed_puzzles >= achievement.condition_value {
                        // Puzzles completed condition
                        criteria_met = true;
                    } else if achievement.condition_type == 2 && progress.total_points >= achievement.condition_value {
                        // Points earned condition
                        criteria_met = true;
                    } else if achievement.condition_type == 3 && progress.highest_difficulty_completed >= achievement.condition_value.try_into().unwrap() {
                    ...
                    if criteria_met {
                        ...
                        // Emit event
                        self.emit(Event::AchievementEarned(AchievementEarned { 
                            user, 
                            achievement_id, 
                            mode_id, 
