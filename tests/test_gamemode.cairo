...
use quest_contract::interfaces::igamemode::{
    IGameModeDispatcher, IGameModeDispatcherTrait,
};
...
fn test_create_custom_game_mode() {
    ...
    let mode_id = contract.create_game_mode(
        'Custom Mode',
...
fn test_create_achievement() {
    ...
    let achievement_id = contract.create_achievement(
        1, // Practice mode ID
        'Test Achievement',
...
fn test_start_puzzle_in_mode() {
    ...
    let (time_limit, reward_multiplier) = contract.start_puzzle_in_mode(
        user,
        1, // Puzzle ID
        1  // Practice Mode ID
    );
...
fn test_complete_puzzle_and_progress() {
    ...
    // Complete the puzzle
    let (adjusted_rewards, achievements) = contract.complete_puzzle_in_mode(
        user,
        1, // Puzzle ID
...
fn test_challenge_mode_reward_modifiers() {
    ...
    set_block_timestamp(1100);
    let (adjusted_rewards, _) = contract.complete_puzzle_in_mode(
        user,
        1, // Puzzle ID
        2, // Challenge Mode
...
fn test_time_attack_mode_reward_modifiers() {
    ...
    let (adjusted_rewards_fast, _) = contract.complete_puzzle_in_mode(
        user,
        1, // Puzzle ID
        3, // Time Attack Mode
        100, // Points
    ...
    let (adjusted_rewards_moderate, _) = contract.complete_puzzle_in_mode(
        user,
        2, // Puzzle ID
        3, // Time Attack Mode
        100, // Points
...
fn test_all_reward_combinations() {
    ...
    assert(time_attack_moderate == 156, 'Time Attack moderate wrong'); // 100 * 1.25 * 1.25 = 156.25 ≈ 156
...
fn test_user_progress_and_achievements() {
    ...
    assert(adjusted_rewards == 100, 'Incorrect adjusted rewards'); // Practice mode has 1x multiplier
...
fn test_reward_modifiers() {
    ...
    assert(time_attack_rewards_fast == 187, 'Time Attack fast rewards incorrect'); // 100 * 1.25 * 1.5 = 187
    ...
    assert(time_attack_rewards_moderate == 156, 'Time Attack moderate rewards incorrect'); // 100 * 1.25 * 1.25 = 156
}